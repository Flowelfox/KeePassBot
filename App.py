...
def stop(bot, update):
    ...
    if user:
        ...
        bot.send_message(chat_id=chat.id, text="Sorry, i don't know who you are, please send me \"/start\"")
...
def database_add(bot, update):
    ...
    file.download(out=input_f)
    input_f.seek(0)

    """Saving to database"""
    user.file = input_f.read()
    user.save()
    ...
    bot.send_message(chat_id=chat.id, text="Did you need file-key for openning you database?", reply_markup=markup)
...
def not_opened(bot, update):
    ...
    if user.key_file_needed and not os.path.exists(TEMP_FOLDER + '/' + update.message.from_user.name + '.key'):
        if not update.message.document:
            bot.send_message(chat_id=chat.id, text="Please send me key-file first")
            return
        ...
        if update.message.document:
            ...
            bot.send_message(chat_id=chat.id, text="Now send me your password")
            return
    ...
    try:
        ...
        if user.notification:
            bot.send_message(chat_id=chat.id, text=exm_mark_emo * 2 + "PLEASE REMOVE YOUR PASSWORD FROM HISTORY" + exm_mark_emo * 2)
        ...
        bot.send_message(chat_id=chat.id, text=str(e))
    ...
    if not user.is_opened:
        if user.password_needed and user.key_file_needed:
            bot.send_message(chat_id=chat.id, text="Please send me key-file and then password to open database")
        elif user.password_needed and not user.key_file_needed:
            bot.send_message(chat_id=chat.id, text="Send me password to open database")
        elif not user.password_needed and user.key_file_needed:
            bot.send_message(chat_id=chat.id, text="Send me key-file to open database")
...
def show_group(bot, update):
    ...
    if data == "Resend":
        # delete previous
        try:
            bot.delete_message(chat_id=chat.id, message_id=user.interface_message_id)
        except Exception as e:
            print(e)

        # send new
        message_text, message_markup = keepass.get_message()
        interface_message = bot.send_message(chat_id=chat.id, text=message_text, reply_markup=message_markup)
    ...
    if data.startswith("Edit_"):
        if KeePass.active_item:
            ...
            if len(uuid) == 24:
                ...
                bot.send_message(chat_id=chat.id, text="Wrong edit uuid, try again.\nIf the problem persists, please write to administrator")
...
def create(bot, update):
    ...
    if args.lower() in "group":
        ...
        bot.send_message(chat_id=chat.id, text="Wrong arguments, choices are: 'Entry', 'Group', 'e', 'g'\nExample: /create Group")
        return
    # Entering in create state
    user.create_state = True
    user.save()
    ...
    if user.notification:
        bot.send_message(chat_id=chat.id, text="Click on the field name to set the value for it.\nSend the text to set the value.\nBold fields are required.\nYou can use the arrows buttons to switch between fields.")
    ...
    if data == "done":
        for field in keepass.add_edit_state.req_fields:
            if not keepass.add_edit_state.fields[field]:
                bot.send_message(chat_id=chat.id, text="Please fill required fields")
                bot.answer_callback_query(update.callback_query.id)
                return
...
def message_in_create(bot, update):
    ...
    if len(update.message.text) > 40:
        bot.send_message(chat_id=chat.id, text="New value too long. Please send text bellow 40 chars")
        return
    try:
...
def close_proces(username, chat_id):
    ...
    user.save()
    try:
        t_bot.delete_message(chat_id=chat_id, message_id=user.interface_message_id)
    except Exception as e:
        print(e)

    t_bot.send_message(chat_id=chat_id, text="Database closed")
    if user.password_needed and user.key_file_needed:
        t_bot.send_message(chat_id=chat_id,
                           text="Please send me key-file and then password to open database")

    elif user.password_needed and not user.key_file_needed:
        t_bot.send_message(chat_id=chat_id, text="Send me password to open database")
    elif not user.password_needed and user.key_file_needed:
        t_bot.send_message(chat_id=chat_id, text="Send me key-file to open database")
...
def delete_database(bot, update):
    ...
    if user.is_opened:
        ...
        user.file = None
        user.save()
        bot.send_message(chat_id=chat.id, text="Your database was deleted")
...
def toogle_notifications(bot, update):
    ...
    user.save()
    if user.notification:
        bot.send_message(chat_id=chat.id, text="Notifications enabled.")
    else:
        bot.send_message(chat_id=chat.id, text="Notifications disabled.")
...
def get_database(bot, update):
    ...
    """Send to user"""
    bot.send_document(chat_id=chat.id, text=exm_mark_emo * 2 + "PLEASE REMOVE DATABASE FILE FROM HISTORY" + exm_mark_emo * 2, document=output)
...
# db exist
# opened
resend_interface_handler = CommandHandler("resend", resend_interface, filters=CustomFilters.user_exists & CustomFilters.password_database_exist & CustomFilters.is_database_opened)

# user exists
# db exist
get_database_handler = CommandHandler("download", get_database, filters=CustomFilters.user_exists & CustomFilters.password_database_exist)
